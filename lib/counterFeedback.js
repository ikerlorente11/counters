const DEFAULT_TAP_SOUND_VOLUME = 0.7;
const DEFAULT_VIBRATION_STRENGTH = 0.65;
const MIN_VIBRATION_DURATION_MS = 10;
const MAX_VIBRATION_DURATION_MS = 42;
const COUNTER_TAP_PLAYER_POOL_SIZE = 1;
...
let tapSoundVolumeLevel = DEFAULT_TAP_SOUND_VOLUME;
let vibrationStrengthLevel = DEFAULT_VIBRATION_STRENGTH;

/**
 * Normalizes a feedback level to the 0.0-1.0 range.
 * @param {number | string | null | undefined} value
 * @param {number} fallback
 * @returns {number}
 */
export function normalizeCounterFeedbackLevel(value, fallback) {
...
/**
 * Returns the current runtime feedback preferences.
 * @returns {{tapSoundVolume: number, vibrationStrength: number}}
 */
export function getCounterFeedbackPreferences() {
  return {
    tapSoundVolume: tapSoundVolumeLevel,
    vibrationStrength: vibrationStrengthLevel,
  };
}

/**
 * Updates runtime feedback preferences for immediate use.
 * @param {{tapSoundVolume?: number | string, vibrationStrength?: number | string}} preferences
 */
export function setCounterFeedbackPreferences(preferences) {
  if (!preferences || typeof preferences !== "object") {
    return;
  }

  if (preferences.tapSoundVolume !== undefined) {
    tapSoundVolumeLevel = normalizeCounterFeedbackLevel(
      preferences.tapSoundVolume,
      DEFAULT_TAP_SOUND_VOLUME,
    );
  }

  if (preferences.vibrationStrength !== undefined) {
    vibrationStrengthLevel = normalizeCounterFeedbackLevel(
...
export async function playCounterTapFeedback() {
  ...
  triggerLightVibration(
    ...
    dependencies.cancelVibration,
    resolveVibrationDurationMs(),
  );

  if (tapSoundVolumeLevel <= 0) {
    return true;
  }
...
async function createCounterTapPlayer(dependencies) {
  ...
  try {
    ...
    // Wait for the audio source to finish loading before warming up.
    // createAudioPlayer loads asynchronously; play() silently fails before isLoaded.
    if (!testDependencies) {
      for (let i = 0; i < 40; i++) {
        if (player.currentStatus?.isLoaded) break;
        await new Promise((r) => setTimeout(r, 50));
      }

      // Warm up: silent play so Android audio HAL initialises its pipeline.
      try {
    ...
    if ("volume" in player) player.volume = tapSoundVolumeLevel;
...
async function restartCounterTapPlayer(player) {
  try {
    if ("volume" in player) {
      player.volume = tapSoundVolumeLevel;
    }
  } catch {
    // Volume update is best-effort.
  }
...
function triggerLightVibration(vibrate, cancelVibration, vibrationDurationMs) {
...
function resolveVibrationDurationMs() {
  if (vibrationStrengthLevel <= 0) {
    return 0;
  }

  return Math.round(
    MIN_VIBRATION_DURATION_MS
    + (MAX_VIBRATION_DURATION_MS - MIN_VIBRATION_DURATION_MS) * vibrationStrengthLevel,
  );
}
...
function resetCounterFeedbackState() {
  ...
  playerPromises = [];
  nextPlayerIndex = 0;
  tapSoundVolumeLevel = DEFAULT_TAP_SOUND_VOLUME;
  vibrationStrengthLevel = DEFAULT_VIBRATION_STRENGTH;
}