...
test("playCounterTapFeedback vibrates and replays the cached audio player", async () => {
  ...
  // Pool size is 1: single player created, reused for both taps
  assert.equal(calls.filter(([name]) => name === "createAudioPlayer").length, 1);
  assert.equal(calls.filter(([name]) => name === "preload").length, 1);
  ...
  // Both taps reuse the single pool player (index 0)
  assert.deepEqual(
...
test("prepareCounterTapFeedback initializes the player without vibrating", async () => {
  ...
  // Pool size 1: one preload and one createAudioPlayer
  assert.deepEqual(calls, [
    "preload",
    "setAudioModeAsync",
